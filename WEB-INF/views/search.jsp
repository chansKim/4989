...
<body>
<%@include file="./member/loginSession.jsp"%>
...
<div class="CEN">
<div class="dashboard">
<table align="center" bgcolor="#efdfff" width="80%">
<tr>
<td>
...
<jsp:useBean id="mDto" class="com.ju.chan.dto.MemberDto"/>


<c:forEach var="key" items="${memlist}" varStatus="loop">
...
<div class="item active">
                	    <div class="col-md-4 col-sm-6">
        				    <div class="block-text rel zmin">
        						...
        						<c:choose>
        							<c:when test="${fn:length(follow) == 0}">
								<button type="submit" id="follow" class="btn btn-default" 
        						onClick="follow('${login.m_mail}','${key.m_mail}')" ><div id='${key.m_mail}'>팔로우</div> </button>  						
      						 	   </c:when>
      						 	   <c:otherwise>
        						<% int type = 0; %>
      						 	  <c:set var="doneLoop" value="false" />
      						 	   <c:forEach var="key5" items="${follow}" >
      						 	   <c:if test="${not doneLoop}">
        						    	<c:choose>
        									<c:when test="${key.m_mail == key5.face_mail}">
        						    		<% type = 1; %>
        						...
        						onClick="unfollow('${login.m_mail}','${key.m_mail}')" > <div id='${key.m_mail}'>언팔로우</div>  </button>
      							   <%
      						   } else {
      							   %>
      							   <button type="submit" id="follow" class="btn btn-default" 
        				    ...
        				    <div class="block-text rel zmin">
        						...
        						onClick="follow('${login.m_mail}','${key.m_mail}')" > <div id='${key.m_mail}'>팔로우</div>   </button>  						
      						   ...
      						   if(type == 1){
      							   %> 
      							   <button type="submit" id="follow" class="btn btn-primary" 
        						unfollow(="follow('${login.m_mail}','${key.m_mail}')" > <div id='${key.m_mail}'>언팔로우</div>  </button>
      							   <%
      						   } else {
      							   %>
      							   <button type="submit" id="follow" class="btn btn-default" 
...
<ul class="nav nav-pills" role="tablist">
  <li role="presentation" class="dropdown">
    <a class="dropdown-toggle" data-toggle="dropdown" href="#">
      포스트별 유형별 필터링 <span class="caret"></span>
    </a>
  ...
  <li role="presentation" class="dropdown">
    <a class="dropdown-toggle" data-toggle="dropdown" href="#">
     인기 포스트 <span class="caret"></span>
    </a>
    <ul class="dropdown-menu" role="menu">
     <li><a href="#">인기 포스트</a></li>
     <li><a href="#">최신 포스트</a></li>
    </ul>
  </li>
</ul>
...
<jsp:useBean id="mDto2" class="com.ju.chan.dto.MemberDto"/>
<c:forEach var="key" items="${postlist}">
<jsp:setProperty property="*" name="mDto2"/>
